    ...
    O: Send + 'static,
{
    ...
    pub fn add_stage<F: 'static + Send + FnMut(O) -> U, U: 'static + Send>(
        ...
        Pipeline { input, last_stage_output: next_stage_output }
    }
    ...
    F: Send + 'static + FnMut(I) -> O
{
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            // report every second
            let dur = e.duration_since(last_report).unwrap_or(Duration::from_secs(0));
            if dur >= Duration::from_secs(1){
                // ms precision is ok
                let dur_in_ms = (1000 * dur.as_secs()) as f64 + dur.subsec_millis() as f64;
                let active_in_ms = (1000 * time_active.as_secs()) as f64 + time_active.subsec_millis() as f64;
                let ops_per_s = n as f64 / dur_in_ms * 1000.0;
                let utilisation = active_in_ms/dur_in_ms * 100.0;
                println!("Stage: {:15} : Processed {} in {:3.3}s ({:9.2}/s); Utilisation: {:3.2}%", name, n, dur_in_ms/1000.0, ops_per_s, utilisation);
...
pub fn new<I,O,F>(
    ...
    Pipeline { input, last_stage_output }
}